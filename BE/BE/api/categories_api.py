...
@category_bp.route('/update/<id>', methods=['POST'])
def UpdateCategory(id):
    cursor = categories.update_category(id, request.get_json())
    result = False
    if(cursor):
        result = True
    else:
...
@category_bp.route('/remove/<id>', methods=['POST'])
def RemoveCategory(id):
    cursor = categories.delete_category(id)
    result = False
    if(cursor):
        result = True
    else:
