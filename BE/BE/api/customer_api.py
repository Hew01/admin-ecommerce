...
@customer_bp.route("/<customerid>/addcoupon/<couponid>", methods=["POST"])
def AddCustomerCoupon(customerid, couponid):
    cursor = customers.add_coupon_to_customer(customerid, couponid)
    result = False
    if(cursor):
        result = True
    else:
...
@customer_bp.route("/<customerid>/removecoupon/<couponid>", methods=["POST"])
def RemoveCustomerCoupon(customerid, couponid):
    cursor = customers.remove_coupon_from_customer(customerid, couponid)
    result = False
    if(cursor):
        result = True
    else:
...
@customer_bp.route("/<id>/addtowishlist/<productid>", methods=["POST"])
def AddCustomerToWishlist(id,productid):
    cursor = customers.add_products_to_wishlist(id, productid)
    result = False
    if(cursor):
        result = True
    else:
...
@customer_bp.route("/<id>/removewishlist/<productid>/", methods=["POST"])
def RemoveCustomerToWishlist(id, productid):
    cursor = customers.remove_products_from_wishlist(id, productid)
    result = False
    if(cursor):
        result = True
    else:
